; Program: Fatorial
; Description: Calculates the factorial of a number and print the result
; Author: Júnior Nascimento EC-3 2018 Matricula:0403490
; Limitations: GET_UDEC takes a number that fits in 4 bytes, 4294967295(decimal),
; otherwise the only limitation is memory, since factorial is a function that grows
; faster the larger the number the more bits we will need to calculate its result.
...
SETMULT: 
    ...
    xor edx,edx                 ; edx = 0 to use in the loop
    .setmult:
    mov eax,[result+edx]        ; get the value of result in the pos (eax = result[i])
...
ADDC:
    xor edx,edx                 ; edx = 0, to control loop
    .add:
    mov eax,[result+edx]        ; get eax = result[i] to sum
    mov ecx,[mutiply+edx]       ; get ecx
    add eax,ecx                 ; add the value
    jc .addcarry                ; if carry resolve it
    .continuecarry:            
    mov [result+edx],eax        ; set the new value of result (result[i] = eax)
    ...
    .addcarry:
    add DWORD[result+edx+incr],0x1 ; add the carry on the result[i+1]
    jmp .continuecarry             ; continue at the stop's
    
PRINT:

    mov edx,size                ; edx = size
    .add:
    PRINT_HEX 4,[result+edx]    ; print(result[i])
    sub edx,incr                ; i --
    cmp edx,0x00                ; (i==0)?
